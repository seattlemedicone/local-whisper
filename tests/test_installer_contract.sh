...
require_text "$REPO_ROOT/install.sh" 'if [[ "$SELECTED_REFINE_STATE" == "on" ]]; then'
require_text "$REPO_ROOT/install.sh" 'Skipping Ollama service and model provisioning while refinement is disabled'
